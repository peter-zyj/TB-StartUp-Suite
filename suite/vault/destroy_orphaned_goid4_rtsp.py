...
def execute(IP,action):    
    ...
    for x in IP:
	    ...
	    y = '.'+x.split('.')[-1]
            ...
            try:
                ssh.sendline('scp %s root@%s:%s%s%s' % (logfn,LocalIP,subdir,'object',y))
                Standard_Reply(ssh,x)
            except:
		print "debug: scp %s root@%s:%s%s%s"  % (logfn,LocalIP,subdir,'object',y)
            ...
            try:
                ssh.sendline('scp %s root@%s:%s/%s%s' % (dbfn,LocalIP,subdir,'db',y))
                Standard_Reply(ssh,x)
            except:
                print "SCP to %s error: " % x + ssh.before[:-1]
                sys.exit(1)
    ...
    cmd7 = '%s/avs_clist %s verbose=2 %s/object.* > verbose.local' % (localPWD,localdb,subdir)
    os.popen(cmd5)
    os.popen(cmd6)
    os.popen(cmd7)
